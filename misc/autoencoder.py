...
class TextAutoencoder(object):
    ...
    def __init__(self, lstm_units, embeddings, go, train=True,
        ...
        with tf.variable_scope('autoencoder') as self.scope:
            ...
            embedded_eos = tf.nn.embedding_lookup(self.embeddings, go_batch)
            embedded_eos = tf.reshape(embedded_eos, [-1, 1, self.embedding_size])
    ...
    def _create_training_tensors(self):
        ...
        decoder_outputs2d = tf.reshape(self.decoder_outputs,[-1, tf.shape(self.decoder_outputs)[-1]])
        labels = tf.reshape(decoder_labels, [-1, 1])
        sampled_loss = tf.nn.sampled_softmax_loss(projection_w_t, projection_b, labels, decoder_outputs2d, 100, self.vocab_size)
